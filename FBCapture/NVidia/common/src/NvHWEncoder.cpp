...
NVENCSTATUS CNvHWEncoder::NvEncRegisterResource(NV_ENC_INPUT_RESOURCE_TYPE resourceType, void* resourceToRegister, uint32_t width, uint32_t height, uint32_t pitch, void** registeredResource) {
  ...
  registerResParams.bufferFormat = NV_ENC_BUFFER_FORMAT_NV12;
...
NVENCSTATUS CNvHWEncoder::CreateEncoder(EncodeConfig *pEncCfg) {
  ...
  m_stCreateEncodeParams.enableEncodeAsync = pEncCfg->enableAsyncMode;
  ...
  m_pEncodeAPI->nvEncGetEncodeCaps(m_hEncoder, m_stCreateEncodeParams.encodeGUID, &stCapsParam, &pEncCfg->enableAsyncMode);
  m_stCreateEncodeParams.enableEncodeAsync = pEncCfg->enableAsyncMode;
...
NVENCSTATUS CNvHWEncoder::ProcessOutput(const EncodeBuffer *pEncodeBuffer, void **buffer, UINT32 *length, UINT64 *timestamp, UINT64 *duration, UINT32 *frameIdx, bool *is_keyframe) {
  NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
...
#if defined(NV_WINDOWS)
    DWORD dwWaitResult = WaitForSingleObject(pEncodeBuffer->stOutputBfr.hOutputEvent, INFINITE);
    switch (dwWaitResult) {
      case WAIT_OBJECT_0:
        break;
      default:
  ...
  lockBitstreamData.doNotWait = m_stCreateEncodeParams.enableEncodeAsync;
  ...
  if (nvStatus == NV_ENC_SUCCESS) {
    ...
    *buffer = malloc(lockBitstreamData.bitstreamSizeInBytes);
    memcpy(*buffer, lockBitstreamData.bitstreamBufferPtr, lockBitstreamData.bitstreamSizeInBytes);
    *length = lockBitstreamData.bitstreamSizeInBytes;
    *timestamp = lockBitstreamData.outputTimeStamp;
    *duration = lockBitstreamData.outputDuration;
    *frameIdx = lockBitstreamData.frameIdx;
    *is_keyframe = lockBitstreamData.pictureType == NV_ENC_PIC_TYPE_I ||
      lockBitstreamData.pictureType == NV_ENC_PIC_TYPE_IDR;
...
NVENCSTATUS CNvHWEncoder::NvEncEncodeFrame(EncodeBuffer *pEncodeBuffer, NvEncPictureCommand *encPicCommand,
  ...
  encPicParams.frameIdx = m_EncodeIdx;
...
NVENCSTATUS CNvHWEncoder::NvEncFinalize(void *hEOSEvent) {
  NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
  NV_ENC_PIC_PARAMS encPicParams;
  memset(&encPicParams, 0, sizeof(encPicParams));
  SET_VER(encPicParams, NV_ENC_PIC_PARAMS);
