...
extern "C" {
  ...
  namespace FBCapture {
    ...
    *
    * NOTE: FBCaptureConfig options are only applicable to video encoding/streaming.
    * If only interested in screenshot, simply pass in an empty FBCaptureConfig object.
    */
    extern FBCAPTURE_API FBCAPTURE_STATUS APIENTRY Create(FBCaptureConfig* config,
    ...
    *
    * This function will work properly iff the FBCapture session status is FBCAPTURE_SESSION_INITIALIZED.
    * On success, this api function sets the session status to FBCAPTURE_SESSION_ACTIVE.
    * On failure, this api function sets the session status to SESSION_FAILURE.
    */
    extern FBCAPTURE_API FBCAPTURE_STATUS APIENTRY StartSession(FBCAPTURE_HANDLE handle,
                                                                const wchar_t* dstUrl);

    /*
    * Function: FBCapture::EncodeFrame()
    *
    ...
    * save to the default directory 'GetCurrentDirectory()\FBCapture\' directory on Windows.
    *
    * This function will only work properly iff FBCapture session status is FBCAPTURE_SESSION_INITIALIZED.
    ...
    */
    extern FBCAPTURE_API FBCAPTURE_STATUS APIENTRY Mute(FBCAPTURE_HANDLE handle,
                                                        bool mute);

    /*
    * Function: UnitySetGraphicsDevice():
    *
    * Called automatically by Unity when the DLL is loaded by the SurroundCapture script.
    * This function figures out the hardware GPU graphics card type (Nvidia vs. AMD)
    * and initializes the corresponding hardware encoder for the capture.
    */
    extern FBCAPTURE_API void APIENTRY UnitySetGraphicsDevice(void* device,
                                                              int deviceType,
                                                              int eventType);
