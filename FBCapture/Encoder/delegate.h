...
#define FBCAPTURE_MAIN_DELEGATE(handle, funcPtr, ...) \
FBCaptureMain* fbCapture = reinterpret_cast<FBCaptureMain*>(handle); \
auto func = DELEGATE(funcPtr, *fbCapture); \
FBCAPTURE_STATUS retStatus = func(__VA_ARGS__); \
if (retStatus != FBCAPTURE_OK) fbCapture->onFailure(retStatus);

#define FBCAPTURE_DELEGATE_TRY(handle, funcPtr, ...) \
__try { FBCAPTURE_MAIN_DELEGATE(handle, funcPtr, __VA_ARGS__); } \
__except (EXCEPTION_EXECUTE_HANDLER) { \
  DEBUG_ERROR_VAR("Unexpected SEH occurred", __FUNCTION__); \
  FBCaptureMain::fbCapture->onFailure(FBCAPTURE_INVALID_FUNCTION_CALL); \
}
